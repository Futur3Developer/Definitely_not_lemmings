...
JoblessLemming::JoblessLemming()
{
    accept_class_change = true;
}

void JoblessLemming::move()
{
...
void JoblessLemming::resolve_collision(QList<QGraphicsItem*> colliding_items)
{
...
void JoblessLemming::bounce_back()
{
    x_axis_speed = -x_axis_speed;
    this -> setPixmap(this->pixmap().transformed(QTransform().scale(-1, 1)));
}

void JoblessLemming::resolve_collision_with_block(QGraphicsItem *block)
{
...
void JoblessLemming::resolve_collision_with_blocker(QGraphicsItem *blocker)
{
...
void JoblessLemming::update_axis_speeds()
{
    if(largest_obstacle_height_prevalence == -1) //There was no collision with block
    {
